...
class FocalLoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        if self.size_average:
            loss = loss.mean()
        return loss.reshape((batch_size,1,256,256))
...
def dice_loss(logit: Tensor, target: Tensor, multiclass: bool = False):
    assert logit.size() == target.size()
    fn = multiclass_dice_coeff if multiclass else dice_coeff
    return 1 - fn(logit, target, reduce_batch_first=True)
...
class DiceLoss(nn.Module):
   ...
   def forward(self, pred_mask_logits, gt_masks):
        # gt_masks = input["masks"]
        # pred_mask_logits = input['full_mask_logits'].sigmoid()

        predict = pred_mask_logits.contiguous().view(pred_mask_logits.shape[0], -1)
